...
export default function Auth() {
  ...
  const handleStudentLogin = async () => {
    ...
    const password = studentPassword.trim() || 'password'; // default password

    const idPattern = /^FCP\/CCS\/20\/\d{4}$/;
    if (!idPattern.test(cleanId)) {
      toast({
        title: 'Invalid Student ID',
        description: 'Use format FCP/CCS/20/1234',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      toast({ title: 'Welcome Student!', description: 'Successfully logged in.' });
      navigate('/student');
    } catch (err: any) {
      toast({
        title: 'Login Failed',
        description: err.message || 'Check your ID/password',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const handleGuestLogin = async () => {
    ...
    try {
      ...
      if (data.user) {
        ...
        toast({ title: 'Welcome Guest!', description: "You're logged in as a guest." });
      }
    } catch (err: any) {
      toast({ title: 'Login Failed', description: err.message || 'Cannot login as guest', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-background to-muted flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <img src={fudLogo} alt="FUD Logo" className="h-24 w-24 object-contain mx-auto mb-4" />
          ...
          <TabsContent value="guest">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Try the system as a guest</CardDescription>
              </CardHeader>
              <CardContent>
